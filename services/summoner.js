...
const getSummonerByName = async (region, summonerName) => {
    ...
    try{
        res = await axios.get(config.summonerByNameUrl(region, summonerName), axiosOptions);
    } catch(err) {
        throw err;
    }
...
const summonerParser = async (req,res,next) => {
    ...
    req.summoner = await getSummonerByName(req.query.region, req.query.summonerName);
    return next();
}
